...
class Clients:
    ...
    def initiate_session(self, target):
        ...
        if response["status"] == "success":
            ...
            print(f"Session established successfully with {self.peer_host} {self.peer_port}")
        else:
    ...
    def listen_for_messages(self):
        ...
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as peer_socket:
            peer_socket.bind(('0.0.0.0', 12348))   # Bind to any free port
    ...
    def send_message(self, message):
        if self.peer_host and self.peer_port:
            with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as peer_socket:
                print("message sending2", type(self.peer_port), type(self.peer_host))
                peer_socket.connect(('192.168.1.242', 12347))
